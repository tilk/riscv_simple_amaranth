            ...
            while True:
                ...
                if stb and cyc:
                    ...
                    if we:
                        ...
                        print("DATA WRITE: %.8x %.8x" % (adr << 2, dat))
                        if adr << 2 == 0xfffffff0:
                            self.assertEqual(dat, 1)
                            yield Passive()
                    else:
                        dat = hex_read(ih, adr)
                        print("DATA READ: %.8x %.8x" % (adr << 2, dat))
...
def test_runner(module, text, data):
    class TestRunner(RunnerBase):
        def runTest(self):
            ...
            sim.add_clock(1e-9)
            sim.add_sync_process(self.text_memory(module, text))
            sim.add_sync_process(self.data_memory(module, data))
