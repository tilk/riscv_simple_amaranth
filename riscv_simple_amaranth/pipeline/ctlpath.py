...
class PipelineControlPath(Elaboratable):
    def __init__(self):
...
#        m.submodules.bus_control = bus_control = PipelineBusControl()
        ...
        m.d.comb += self.stall.eq(self.want_stall)

        m.d.comb += self.insn_request.eq(~self.stall)
        m.d.comb += self.step.eq((~self.insn_request | self.insn_valid) & (~self.mem_request | self.mem_valid))
        m.d.comb += self.pc_we.eq(self.step & self.insn_request)
